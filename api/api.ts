...
export async function PUT(resource: string, body: Record<string, unknown>): Promise<unknown> {
  const parts = resource.split('/').filter(Boolean);

  // Singletons
  if (parts.length === 1) {
    switch (parts[0]) {
      case 'company-settings': return db().companySettings.put(body);
      case 'account': return db().account.put(body);
    }
  }
  ...
  if (parts.length === 2) {
    const [collection, id] = parts;
    switch (collection) {
      case 'users': return db().users.put(id!, body);
      case 'ideas': return db().ideas.put(id!, body);
      case 'projects': return db().projects.put(id!, body);
      case 'edges': return db().edges.put(id!, body);
      case 'activities': return db().activities.put(id!, body);
      case 'notifications': return db().notifications.put(id!, body);
      case 'crunch-columns': return db().crunchColumns.put(id!, body);
      case 'processes': return db().processes.put(id!, body);
      case 'notification-prefs': return db().notificationPrefs.put(id!, body);
    }
  }
  ...
  if (parts.length === 3) {
    const [parent, parentId, child] = parts;
    if (parent === 'ideas' && child === 'score') {
      return db().ideaScores.put(parentId!, body);
    }
  }
  ...
  if (parts.length === 4) {
    ...
    if (parent === 'projects') {
      switch (child) {
        case 'team': return db().projectTeam.put(parentId!, childId!, body);
        case 'milestones': return db().milestones.put(childId!, { ...body, project_id: parentId });
        case 'tasks': return db().projectTasks.put(childId!, { ...body, project_id: parentId });
        case 'discussions': return db().discussions.put(childId!, { ...body, project_id: parentId });
        case 'versions': return db().projectVersions.put(childId!, { ...body, project_id: parentId });
        case 'clarifications': return db().clarifications.put(childId!, { ...body, project_id: parentId });
      }
    }
    if (parent === 'edges') {
      if (child === 'outcomes') return db().edgeOutcomes.put(childId!, { ...body, edge_id: parentId });
    }
  }
  ...
  if (parts.length === 6) {
    ...
    if (parent === 'edges' && child === 'outcomes' && subchild === 'metrics') {
      return db().edgeMetrics.put(subId!, body);
    }
  }
