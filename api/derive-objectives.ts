...
// (empty when the org has no objective document pairs).
// StateEntity wire only (no field_values) — trio-family
// collection history posture.
export async function deriveObjectiveHistories(
    db: DbAdapter,
    organization: Id,
): Promise<StateEntity[]> {
    const prefix = objectivesUriPrefix(organization);
    ...
    for (const [objectiveId, entityPairs] of byObjective) {
        rows.push(...stateHistoryFrom(
            documentLifecycleEvents(entityPairs),
            objectiveId,
        ));
    }
    // Per-entity ASC from stateHistoryFrom; re-sort the bulk
    // (at, id) ASC then reverse so the wire is DESC overall.
