...
export async function init(): Promise<void> {
    ...
    const grid4 = 'display:grid'
        + ';grid-template-columns:repeat(4,1fr)'
        + ';gap:1.5rem';

    const grid5 = 'display:grid'
        + ';grid-template-columns:repeat(5,1fr)'
        + ';gap:1.5rem';

    const grid6 = 'display:grid'
    ...
    const errSoftBorder = 'padding:1rem'
    ...
    const warnSoftBorder = 'padding:1rem'
        + ';border-radius:var(--radius)'
    ...
    const uploadFailStyle = 'padding:1.5rem'
    ...
    const softSuccBtn = 'background:'
        + 'hsl(var(--success-soft))'
        + ';color:hsl(var(--success-text))';

    const softWarnBtn = 'background:'
        + 'hsl(var(--warning-soft))'
        + ';color:hsl(var(--warning-text))';

    const softErrBtn = 'background:'
        + 'hsl(var(--error-soft))'
        + ';color:hsl(var(--error-text))';

    const accentBadgeStyle = 'display:inline-flex'
        + ';align-items:center'
        + ';gap:0.25rem'
    ...
    const iconColStyle = 'display:flex'
        + ';flex-direction:column'
        + ';align-items:center'
        + ';gap:0.5rem'
        + ';width:3.5rem';

    const iconSizeColStyle = 'display:flex'
        + ';flex-direction:column'
        + ';align-items:center'
        + ';gap:0.75rem';

    const borderTopStyle = 'border-top:'
        + '1px solid hsl(var(--border))';

    const spaceBarStyle = 'height:1rem'
        + ';background:hsl(var(--primary))'
        + ';border-radius:0.25rem';

    const doCardStyle = 'padding:1.5rem'
        + ';border-color:'
        + 'hsl(var(--success) / 0.3)'
        + ';background:'
        + 'hsl(var(--success-soft)/0.3)';

    const dontCardStyle = 'padding:1.5rem'
        + ';border-color:'
        + 'hsl(var(--error)/0.3)'
        + ';background:'
        + 'hsl(var(--error-soft)/0.3)';

    const doColStyle = 'display:flex'
        + ';flex-direction:column'
        + ';gap:0.5rem';

    const fullPageErrDesc =
        'max-width:28rem'
      ...
      <section style="${sectionStyle}">
        <div>
          <h2 class="${''
              ...
              }font-display">Blue Scale</h2>
          <p class="text-muted mt-1">${''
              }Full blue color ramp for ${''
              }nuanced UI design</p>
        </div>
        <div style="${grid5}"
            class="stats-grid">
      ...
      <section style="${sectionStyle}">
        <div>
          <h2 class="${''
              ...
              }font-display">Typography</h2>
          <p class="text-muted mt-1">${''
              }IBM Plex Sans for display, ${''
      ...
      <section style="${sectionStyle}">
        ...
        <div style="${grid3}"
          ...
          <div class="card card-flat"
            ...
            <p class="text-sm text-muted">${''
                }A minimal card with just ${''
                }a border, suitable for ${''
                }secondary content.</p>
          </div>
          <div class="card"
              style="padding:1.5rem">
      ...
      <section style="${sectionStyle}">
        ...
        <div class="card"
          ...
          <div class="${''
            ...
            ${[16, 20, 24, 48].map(
                size => html`
              <div style="${iconSizeColStyle}">
                <div style="${''
                    }color:hsl(${''
                    }var(--primary))">${''
                    }${iconSparkles(size)}</div>
                <code class="${''
                    }text-xs text-muted">${''
                    }${size}px</code>
              </div>
          ...
          <div class="card"
            ...
            <div class="text-sm"
              ...
              <p>${iconX(
                  14,
                  'text-error',
              )} Skip focus states on ${''
                  }interactive ${''
                  }elements</p>
              <p>${iconX(
                  14,
                  'text-error',
              )} Mix typography scales ${''
                  }inconsistently</p>
            </div>
          </div>
        </div>
