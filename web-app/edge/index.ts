...
function buildEdgePage(ideaId: string): SafeHtml {
    ...
    const statusLabel = completion.isComplete
        ...
        : completion.completionPercent > 0
            ? 'In Progress'
            : 'Incomplete';
    const statusClassName = completion.isComplete
        ? 'badge-success'
        : completion.completionPercent > 0
            ? 'badge-warning'
            : 'badge-error';
          ...
          <div class="p-4 rounded-lg"
            ...
            <div style="padding-left:2.25rem">
              <div class="${'flex items-center '
                ...
                <button class="${'btn btn-ghost '
                    + 'btn-xs gap-1'}"
                  data-add-metric="${outcome.id}"
                  >${iconPlus(12)} Add Metric
                </button>
              </div>
              ${outcome.metrics.length === 0
                ? html`<p class="${'text-xs '
                    ...
                    >Add at least one metric to
                    measure this outcome</p>`
                : html`${outcome.metrics.map(
                    metric => html`
    ...
    <div>
      <div class="${'flex items-center '
        ...
        <button class="btn btn-hero gap-2"
          id="edge-save-btn"
          ${trusted(
      ...
      <div class="card p-4 mb-6">
        ...
        <div class="${'flex flex-wrap '
          ...
          <div class="${'flex items-center '
              + 'gap-1 text-xs'}"
            >${buildCompletionIcon(
                completion.allOutcomesHaveMetrics
                && completion.hasOutcomes,
            )} <span class="${
                completion.allOutcomesHaveMetrics
                && completion.hasOutcomes
        ...
        <div style="${'display:flex;'
          ...
          <div class="card p-5">
            <h3 class="${'font-display '
                + 'font-semibold mb-4 '
                + 'flex items-center gap-2'}"
              >${iconTrendingUp(20, 'text-primary')}
              Expected Impact</h3>
            <div class="impact-grid">
              <div>
                <label class="${'label text-xs '
                  ...
                  >${iconClock(14)}
                  Short-term (0-3 months)</label>
                <textarea
                  class="textarea text-sm"
                  rows="4"
                  id="edge-impact-short-term"
              ...
              <div>
                <label class="${'label text-xs '
                    ...
                    + 'flex items-center gap-1'}"
                  >${iconClock(14)}
                  Mid-term (3-12 months)</label>
                <textarea
                  class="textarea text-sm"
                  rows="4"
                  id="edge-impact-mid-term"
                  placeholder="${'Expected '
                      + 'impact over '
                      + '3-12 months...'}"
                  >${edgeData.impact.midTerm}
              ...
              <div>
                <label class="${'label text-xs '
                    ...
                    + 'flex items-center gap-1'}"
                  >${iconClock(14)}
                  Long-term (12+ months)</label>
                <textarea
                  class="textarea text-sm"
                  rows="4"
                  id="edge-impact-long-term"
                  placeholder="${'Expected '
                      + 'impact after '
                      + '12 months...'}"
                  >${edgeData.impact.longTerm}
          ...
          <div class="card p-5">
            <h3 class="${'font-display '
              ...
              Confidence &amp; Ownership</h3>
            <div style="${'display:grid;'
                + 'grid-template-columns:'
                + '1fr 1fr;gap:1rem'}">
              <div>
                <label class="${'label text-xs '
                    + 'text-muted mb-2 block'}"
                  >Confidence Level</label>
                <select class="input"
                  id="edge-confidence-select">
                  <option value=""
