...
function buildDataTypeIcon(type: string): SafeHtml {
    switch (type) {
        ...
        case 'date':
            return iconCalendar(16, 'text-muted');
        case 'boolean':
            return iconToggleLeft(16, 'text-muted');
        default:
        ...
        <div class="flex items-start gap-3">
          <div
            style="width:2.5rem;height:2.5rem;
                   ...
                   flex-shrink:0"
          >
            ${iconHelpCircle(20, 'text-warning')}
          </div>
          <div>
            ...
            <p class="text-sm text-muted">
              ${'After upload, we\'ll ask you'
                + ' simple questions about each'
                + ' column in your data. You\'ll'
                + ' tell us what abbreviations'
                + ' mean, what the data'
                + ' represents, and any'
                + ' business rules.'}
            </p>
      ...
      >
        ...
        ${state.columns.map(column => {
            ...
            const isColumnLabeled =
                ...
                && (!column.isAcronym
                    || column.acronymExpansion);
            const borderStyle = isColumnLabeled
                ? 'border-color:hsl(var('
                  + '--success) / 0.3);'
                  + 'background:hsl(var('
                  + '--success) / 0.03)'
                : '';
            const iconBg = isColumnLabeled
                ? 'background:hsl(var('
                  + '--success) / 0.1)'
                : 'background:hsl(var(--muted))';
            return html`
    ...
    >
      ...
      <p
        ...
        style="max-width:28rem;
               ...
               margin-right:auto"
      >
        ${'Your data has been processed and is'
          + ' ready to use. All columns have'
          + ' been labeled and documented.'}
      </p>
      <div
        class="flex items-center
...
function syncFormFields(): void {
    document.querySelectorAll<
        ...
        | HTMLTextAreaElement
    >(
        '[data-column-id][data-field-name]',
    ).forEach(el => {
        const columnId =
...
function buildCrunchPage(): SafeHtml {
    ...
    <div style="max-width:56rem;margin:0 auto">
      <div style="margin-bottom:2rem">
        ...
        >
          ${'Upload your business data and help'
            + ' us understand it. We\'ll guide'
            + ' you through labeling columns,'
            + ' explaining acronyms, and'
            + ' defining what everything means'
            + ' in plain language.'}
        </p>
      </div>
