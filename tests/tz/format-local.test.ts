...
test('runs under the expected UTC-10 zone', () => {
    const offsetMin = new Date('2026-06-01T00:00:00Z')
        .getTimezoneOffset();
    assert.equal(offsetMin, 600);
});
...
test('formatDate renders an instant in local time', () => {
    assert.equal(formatDate(INSTANT), 'Jan 1, 2026');
});
...
test('formatCalendarDate renders the zone-neutral day', () => {
    assert.equal(formatCalendarDate(INSTANT), 'Jan 2, 2026');
});
