...
Deno.test('toDate of an HTTP-date is timezone-independent', () => {
...
Deno.test('parses an RFC 850 date with the 50-year window', () => {
...
Deno.test('RFC 850 two-digit year resolves into this century', () => {
    const reference = new Date(Date.UTC(2026, 0, 1));
    const date = parseHttpDate(
        'Tuesday, 06-Nov-29 00:00:00 GMT', reference,
    );
    assertStrictEquals(date.getUTCFullYear(), 2029);
});

Deno.test('parses an asctime date with a space-padded day', () => {
...
Deno.test('rejects a non-HTTP-date', () => {
    assertThrows(
