import { test } from 'node:test';
import { strict as assert } from 'node:assert';
import { MemoryStorageBackend }
    from '../api/backend-memory.ts';
import { StateStore } from '../api/store-state.ts';
import {
    EntityNotFoundError,
    LedgerImmutabilityError,
    backendRunner,
} from '../api/db.ts';
import { validateStateEntity } from '../api/validators.ts';

// Phase Final Stage B: states table retired from TABLE_NAMES.
// Pin StateStore against a local ensureTables(['states'])
// backend so the class stays covered until Task 5 deletes it
// with EntityStore.

async function freshStates(): Promise<StateStore> {
    const backend = new MemoryStorageBackend();
    await backend.ensureTables(['states']);
    return new StateStore(
        backendRunner(backend), 'states',
        validateStateEntity,
    );
}

test(
    'put round-trip writes and reads back the row',
    async () => {
        const states = await freshStates();
        const written = await states.put('s1', {
            entity_id: 'e1',
            state: 'active',
            member_id: 'w1',
            at: '2026-01-01T00:00:00.000000Z',
        });
        assert.equal(written.id, 's1');
        assert.equal(written.entity_id, 'e1');
        assert.equal(written.state, 'active');
        assert.equal(written.member_id, 'w1');
        const read = await states.getById('s1');
        assert.deepEqual(read, written);
    },
);

test(
    'identical re-put is an idempotent ok',
    async () => {
        const states = await freshStates();
        const fields = {
            entity_id: 'e1',
            state: 'active',
            member_id: 'w1',
            at: '2026-01-01T00:00:00.000000Z',
        };
        await states.put('s1', fields);
        const again = await states.put('s1', fields);
        assert.equal(again.id, 's1');
        const all = await states.getAll();
        assert.equal(all.length, 1);
        assert.deepEqual(all[0], { ...fields, id: 's1' });
    },
);

test(
    'different re-put throws and leaves the event intact',
    async () => {
        const states = await freshStates();
        const fields = {
            entity_id: 'e1',
            state: 'active',
            member_id: 'w1',
            at: '2026-01-01T00:00:00.000000Z',
        };
        await states.put('s1', fields);
        await assert.rejects(
            () => states.put('s1', {
                ...fields,
                state: 'deleted',
            }),
            (err: unknown) =>
                err instanceof LedgerImmutabilityError,
        );
        const read = await states.getById('s1');
        assert.deepEqual(read, { ...fields, id: 's1' });
    },
);

test(
    'getById throws EntityNotFoundError for unknown id',
    async () => {
        const states = await freshStates();
        await assert.rejects(
            () => states.getById('missing'),
            (err: unknown) =>
                err instanceof EntityNotFoundError,
        );
    },
);

test('getAll returns every written row', async () => {
    const states = await freshStates();
    await states.put('s1', {
        entity_id: 'e1',
        state: 'a',
        member_id: 'w1',
        at: '2026-01-01T00:00:00.000000Z',
    });
    await states.put('s2', {
        entity_id: 'e2',
        state: 'b',
        member_id: 'w1',
        at: '2026-01-02T00:00:00.000000Z',
    });
    const all = await states.getAll();
    assert.equal(all.length, 2);
    const ids = all.map(r => r.id).sort();
    assert.deepEqual(ids, ['s1', 's2']);
});

test(
    'record writes nowUtc; later record has later at',
    async () => {
        const states = await freshStates();
        await states.postEvent(
            's1', 'e1', 'a', 'w1',
            '2026-01-01T00:00:00.000000Z',
        );
        await states.postEvent(
            's2', 'e1', 'b', 'w1',
            '2026-01-01T00:00:01.000000Z',
        );
        const first = await states.getById('s1');
        const second = await states.getById('s2');
        assert.ok(second.at > first.at);
        assert.equal(first.entity_id, 'e1');
        assert.equal(second.entity_id, 'e1');
        assert.equal(first.state, 'a');
        assert.equal(second.state, 'b');
        assert.equal(first.member_id, 'w1');
    },
);

test(
    'currentFor returns the latest of three events',
    async () => {
        const states = await freshStates();
        await states.put('s1', {
            entity_id: 'e1',
            state: 'a',
            member_id: 'w1',
            at: '2026-01-01T00:00:00.000000Z',
        });
        await states.put('s2', {
            entity_id: 'e1',
            state: 'b',
            member_id: 'w1',
            at: '2026-01-03T00:00:00.000000Z',
        });
        await states.put('s3', {
            entity_id: 'e1',
            state: 'c',
            member_id: 'w1',
            at: '2026-01-02T00:00:00.000000Z',
        });
        const current =
            await states.getCurrentFor('e1');
        assert.ok(current !== null);
        assert.equal(current.id, 's2');
        assert.equal(current.state, 'b');
    },
);

test(
    'currentFor returns null for unknown entity_id',
    async () => {
        const states = await freshStates();
        await states.put('s1', {
            entity_id: 'e1',
            state: 'a',
            member_id: 'w1',
            at: '2026-01-01T00:00:00.000000Z',
        });
        const current =
            await states.getCurrentFor('unknown');
        assert.equal(current, null);
    },
);

test(
    'allFor returns events for one entity_id sorted '
        + 'ascending by at',
    async () => {
        const states = await freshStates();
        await states.put('s1', {
            entity_id: 'e1',
            state: 'b',
            member_id: 'w1',
            at: '2026-01-02T00:00:00.000000Z',
        });
        await states.put('s2', {
            entity_id: 'e2',
            state: 'x',
            member_id: 'w1',
            at: '2026-01-01T00:00:00.000000Z',
        });
        await states.put('s3', {
            entity_id: 'e1',
            state: 'a',
            member_id: 'w1',
            at: '2026-01-01T00:00:00.000000Z',
        });
        await states.put('s4', {
            entity_id: 'e1',
            state: 'c',
            member_id: 'w1',
            at: '2026-01-03T00:00:00.000000Z',
        });
        const events = await states.getAllFor('e1');
        assert.equal(events.length, 3);
        assert.deepEqual(
            events.map(e => e.state),
            ['a', 'b', 'c'],
        );
        assert.deepEqual(
            events.map(e => e.at),
            [
                '2026-01-01T00:00:00.000000Z',
                '2026-01-02T00:00:00.000000Z',
                '2026-01-03T00:00:00.000000Z',
            ],
        );
    },
);

test(
    'allFor breaks at-ties by id, agreeing with currentFor',
    async () => {
        // Same at, inserted out of id order. The (at, id) total
        // order sorts ties by id ascending and agrees with
        // getCurrentFor: the last event IS the current one,
        // independent of insertion order. Caller-minted at can
        // tie; the old monotonic server stamp never did.
        const states = await freshStates();
        const at = '2026-01-01T00:00:00.000000Z';
        for (const id of ['sc', 'sb', 'sa']) {
            await states.put(id, {
                entity_id: 'e1',
                state: id,
                member_id: 'w1',
                at,
            });
        }
        const events = await states.getAllFor('e1');
        assert.deepEqual(
            events.map(e => e.id),
            ['sa', 'sb', 'sc'],
        );
        const current = await states.getCurrentFor('e1');
        assert.equal(events.at(-1)!.id, current!.id);
    },
);

test(
    'allFor returns empty array for unknown entity_id',
    async () => {
        const states = await freshStates();
        await states.put('s1', {
            entity_id: 'e1',
            state: 'a',
            member_id: 'w1',
            at: '2026-01-01T00:00:00.000000Z',
        });
        const events =
            await states.getAllFor('unknown');
        assert.deepEqual(events, []);
    },
);

const AT1 = '2026-01-01T00:00:00.000000Z';
const AT2 = '2026-01-01T00:00:00.000001Z';

test('postEvent stamps the caller-supplied at', async () => {
    const states = await freshStates();
    await states.postEvent('s1', 'e1', 'active', 'w1', AT1);
    const row = await states.getById('s1');
    assert.equal(row.at, AT1);
});

test('postEvent replays identically as a no-op', async () => {
    const states = await freshStates();
    await states.postEvent('s1', 'e1', 'active', 'w1', AT1);
    await states.postEvent('s1', 'e1', 'active', 'w1', AT1);
    const all = await states.getAllFor('e1');
    assert.equal(all.length, 1);
});

test('postEvent rejects a conflicting at on one id', async () => {
    const states = await freshStates();
    await states.postEvent('s1', 'e1', 'active', 'w1', AT1);
    await assert.rejects(
        () => states.postEvent('s1', 'e1', 'active', 'w1', AT2),
        (e) => e instanceof LedgerImmutabilityError,
    );
});
