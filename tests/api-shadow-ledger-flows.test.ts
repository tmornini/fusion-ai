...
// versions-publish RETIRED (Phase 15 Task 7). Undo still
// pins operation-addressed uriId-empty behavior.
test('undo is operation-addressed: uriId stays empty, never'
+ ' chains', async () => {
    const db = await freshDb();
    const token = await organizationToken();
...
// Task 4 (R1/E5) + Phase 15 Task 7: redo is a client document
// PUT only — versions POST retired with the routes.
test('redo document-PUT lands at the flow\'s own address',
async () => {
    const db = await freshDb();
    const token = await organizationToken();
    ...
    const documentRows = (await db.requests.getAll())
        .filter(r => r.uri_prefix === '/organizations/1/flows/'
            && r.uri_id === 'flow-9');
    // create op + synthesized document + this redo save.
