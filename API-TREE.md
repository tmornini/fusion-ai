...
└─|─ (lifecycle history)                       • RECONCILED (states-address retirement as-built): bulk lifecycle collection RETIRED; per-entity history lives on GET <family>/:id/history (derived + pair-plane fence — foreign → 403, orphan/own pass); bulk legs on GET work-orders/history and GET objectives/history; field values fold on WO transition history (visibility via stateEventVisibilityFor 3-tier — own → 200 rows; foreign → 403; orphan → 404); bare states/:id + per-entity current-state alias + nested field-values writes RETIRED → router 404 for authenticated callers (unauthenticated → 401 first); states TABLE DELETED (Phase Final); lifecycle writes ride document-trio PUTs + named ops (work-order create/claim/transition/release, invitations) — see Phase 15 + Phase Final + honest HTTP status covenant
